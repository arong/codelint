...
void format_output_text(const std::vector<codelint::lint::LintIssue>& issues) {
  ...
  if (count > 0) {
    std::cout << "Found " << count << " singleton pattern(s)\n";
  } else {
    std::cout << "No singleton patterns found\n";
  }
}
...
int find_singleton(const GlobalOptions& opts, const FindSingletonOptions& singleton_opts) {
  ...
  std::vector<std::string> files = collect_cpp_files(singleton_opts.path);

  if (files.empty()) {
    if (opts.output_json) {
      std::cout << "{\"issues\": []}\n";
    } else {
      std::cerr << "Error: No C++ files found in " << singleton_opts.path << std::endl;
    }
    return 1;
  }
  ...
  SingletonChecker checker(scope);
