...
void NullPointerDereferenceCheck::checkBlockForDereferences(const CFGBlock* Block,
                                                            const BlockState& EntryState,
                                                            ASTContext* Ctx) {
  if (Block == nullptr || Ctx == nullptr) {
    return;
  }

  // Track state as we process statements sequentially
  BlockState CurrentState = EntryState;
  ...
  for (const CFGElement& Elem : *Block) {
    ...
    // First, update state based on this statement (like transferBlock does)
    if (const auto* DS = dyn_cast<DeclStmt>(S)) {
      for (const Decl* D : DS->getDeclGroup()) {
        if (const auto* VD = dyn_cast<VarDecl>(D)) {
          if (isPointerVariable(VD) && VD->hasInit()) {
          ...
          // Check for dereferences in the initializer expression
          if (VD->hasInit()) {
            checkExprForDereferences(VD->getInit(), CurrentState, Ctx);
          }
        }
      }
    }
    ...
    // Now check for dereferences using the current state
    ...
    if (const auto* BO = dyn_cast<BinaryOperator>(S)) {
      if (BO->isAssignmentOp() && isPointerDereference(BO->getLHS())) {
        ...
        } else if (const auto* ME = dyn_cast<MemberExpr>(BO->getLHS())) {
          const Expr* Base = ME->getBase()->IgnoreImplicit();
          if (Base->getType()->isPointerType()) {
            PointerExpr = Base;
          }
        }
    ...
    for (const Expr* PointerExpr : Derefs) {
      ...
      if (CurrentState.VarStates.contains(PointerVar)) {
        PointerState = CurrentState.VarStates.find(PointerVar)->second;
      }
...
void NullPointerDereferenceCheck::checkExprForDereferences(const Expr* E, const BlockState& State,
                                                           ASTContext* Ctx) {
  if (E == nullptr || Ctx == nullptr) {
    return;
  }

  E = E->IgnoreImplicit();

  // Check for direct dereference
  if (const auto* UO = dyn_cast<UnaryOperator>(E)) {
    if (UO->getOpcode() == UO_Deref) {
      const Expr* SubExpr = UO->getSubExpr()->IgnoreImplicit();
      const VarDecl* PointerVar = getReferencedVar(SubExpr);
      if (PointerVar != nullptr && State.VarStates.contains(PointerVar)) {
        NullState PointerState = State.VarStates.find(PointerVar)->second;
        if (PointerState == NullState::Null || PointerState == NullState::MayBeNull) {
          reportDereference(UO->getExprLoc(), SubExpr, PointerState);
        }
      }
    }
    // Recursively check sub-expression
    checkExprForDereferences(UO->getSubExpr(), State, Ctx);
    return;
  }

  // Check for member access via pointer
  if (const auto* ME = dyn_cast<MemberExpr>(E)) {
    const Expr* Base = ME->getBase()->IgnoreImplicit();
    if (Base->getType()->isPointerType()) {
      const VarDecl* PointerVar = getReferencedVar(Base);
      if (PointerVar != nullptr && State.VarStates.contains(PointerVar)) {
        NullState PointerState = State.VarStates.find(PointerVar)->second;
        if (PointerState == NullState::Null || PointerState == NullState::MayBeNull) {
          reportDereference(ME->getExprLoc(), Base, PointerState);
        }
      }
    }
    // Recursively check base
    checkExprForDereferences(ME->getBase(), State, Ctx);
    return;
  }

  // Check for array subscript on pointer
  if (const auto* ASE = dyn_cast<ArraySubscriptExpr>(E)) {
    ...
    if (Base->getType()->isPointerType()) {
      ...
      if (PointerVar != nullptr && State.VarStates.contains(PointerVar)) {
        NullState PointerState = State.VarStates.find(PointerVar)->second;
        if (PointerState == NullState::Null || PointerState == NullState::MayBeNull) {
          reportDereference(ASE->getExprLoc(), Base, PointerState);
        }
      }
    }
    // Recursively check base and index
    checkExprForDereferences(ASE->getBase(), State, Ctx);
    checkExprForDereferences(ASE->getIdx(), State, Ctx);
    return;
  }

  // Check for binary operators (e.g., assignment, arithmetic)
  if (const auto* BO = dyn_cast<BinaryOperator>(E)) {
    if (BO->isAssignmentOp() && isPointerDereference(BO->getLHS())) {
      const Expr* LHS = BO->getLHS()->IgnoreImplicit();
      if (const auto* UO = dyn_cast<UnaryOperator>(LHS)) {
        if (UO->getOpcode() == UO_Deref) {
          ...
          if (PointerVar != nullptr && State.VarStates.contains(PointerVar)) {
            NullState PointerState = State.VarStates.find(PointerVar)->second;
            if (PointerState == NullState::Null || PointerState == NullState::MayBeNull) {
              reportDereference(LHS->getExprLoc(), SubExpr, PointerState);
            }
          }
        }
      }
      if (const auto* ME = dyn_cast<MemberExpr>(LHS)) {
        ...
        if (Base->getType()->isPointerType()) {
          ...
          if (PointerVar != nullptr && State.VarStates.contains(PointerVar)) {
            NullState PointerState = State.VarStates.find(PointerVar)->second;
            if (PointerState == NullState::Null || PointerState == NullState::MayBeNull) {
              reportDereference(LHS->getExprLoc(), Base, PointerState);
            }
          }
        }
      }
    ...
    checkExprForDereferences(BO->getLHS(), State, Ctx);
    checkExprForDereferences(BO->getRHS(), State, Ctx);
    return;
  }
  ...
  if (const auto* CE = dyn_cast<CallExpr>(E)) {
    for (const Expr* Arg : CE->arguments()) {
      checkExprForDereferences(Arg, State, Ctx);
    }
    return;
  }

  // Check for cast expressions
  if (const auto* CastE = dyn_cast<CastExpr>(E)) {
    checkExprForDereferences(CastE->getSubExpr(), State, Ctx);
    return;
  }

  // Check for conditional operator
  if (const auto* CO = dyn_cast<ConditionalOperator>(E)) {
    checkExprForDereferences(CO->getCond(), State, Ctx);
    checkExprForDereferences(CO->getTrueExpr(), State, Ctx);
    checkExprForDereferences(CO->getFalseExpr(), State, Ctx);
    return;
  }
}
