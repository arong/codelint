...
void test_multiple_declarators() {
  int a, b,
      c; // All three should trigger msgs
  ...
  double x = 1.0, y, z = 3.0; // Only y should trigger msg
  // CHECK-MESSAGES: :[[@LINE-1]]:19: error: variable is not initialized  [codelint-init]
  char *p1, *p2, *p3; // All three should trigger msgs
  ...
  int arr1[5], arr2[10]; // Both should trigger C-style array msgs
  ...
  float f1{}, f2, f3{}; // Only f2 should trigger msg
...
void test_single_declarators() {
  int single; // Should trigger msg
              // CHECK-MESSAGES: :[[@LINE-1]]:7: error: variable is not initialized  [codelint-init]
  int initialized{}; // Should NOT trigger msg
}

class TestClass {
public:
  void method() {
    int m1, m2, m3; // All three should trigger msgs
    // CHECK-MESSAGES: :[[@LINE-1]]:9: error: variable is not initialized  [codelint-init]
    ...
    bool b1 = true, b2; // Both should trigger msgs
    // CHECK-MESSAGES: :[[@LINE-1]]:21: error: variable is not initialized  [codelint-init]
  }
};

int global1, global2, global3; // All three should trigger msgs
...
static int s1, s2{}; // Only s1 should trigger msg
