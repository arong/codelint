...
// CHECK-MESSAGES: :21:7: error: C-style array is not initialized  [codelint-init]

void function_test() {
  int local_var;     // This SHOULD trigger a warning
                     // CHECK-MESSAGES: :25:7: error: variable is not initialized  [codelint-init]
  char char_arr[20]; // This SHOULD trigger a C-style array warning
// CHECK-MESSAGES: :27:8: error: C-style array is not initialized  [codelint-init]
...
// CHECK-MESSAGES: :46:8: error: variable is not initialized  [codelint-init]

// Regular C-style arrays (should trigger warnings)
bool bool_flags[25]; // This SHOULD trigger a C-style array warning
// CHECK-MESSAGES: :50:6: error: C-style array is not initialized  [codelint-init]
...
// CHECK-FIXES: MACRO_VAR; // This should NOT trigger a warning
// CHECK-FIXES: #define DECLARE_VAR(type, name) type name
// CHECK-FIXES: DECLARE_VAR(int, macro_y); // This should NOT trigger a warning
// CHECK-FIXES: #define CREATE_ARRAY(type, name, size) type name[size];
// CHECK-FIXES: CREATE_ARRAY(int, macro_arr, 5); // This should NOT trigger a warning
// CHECK-FIXES: int regular_var{};                    // This SHOULD trigger a warning
// CHECK-FIXES: int regular_array[5]{};               // This SHOULD trigger a C-style array warning
// CHECK-FIXES: int initialized_array[5]{};           // This SHOULD trigger a warning (suggest
